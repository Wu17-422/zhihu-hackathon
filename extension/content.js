// 犀利评手 · 内容脚本（契约 v1 对齐，完整版）
// 功能：评论/回答逻辑分析 + 立场比例条 + 相关帖子 + 思维导图 + 问题全扫 + 写评论实时润色
// 后端：http://localhost:8000

const API = "http://localhost:8000";

const CSS = `
  #zha-real-btn,#zha-mindmap-btn,#zha-polish-btn{position:fixed;right:24px;z-index:2147483000;border:none;border-radius:22px;font-size:14px;font-weight:600;cursor:pointer;box-shadow:0 4px 14px rgba(0,0,0,.2)}
  #zha-real-btn{bottom:24px;padding:12px 20px;background:#0066ff;color:#fff}
  #zha-mindmap-btn{bottom:78px;padding:10px 16px;background:#00897b;color:#fff}
  #zha-polish-btn{bottom:132px;padding:10px 16px;background:#7c4dff;color:#fff}
  #zha-real-panel,#zha-polish-panel{position:fixed;top:0;right:0;width:430px;height:100vh;background:#fff;z-index:2147483001;box-shadow:-4px 0 20px rgba(0,0,0,.15);overflow-y:auto;font-size:13px;color:#1a1a1a;line-height:1.6;padding:16px;box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,"PingFang SC","Microsoft YaHei",sans-serif}
  ...
  .zr-polish-bar{display:flex;align-items:center;gap:10px;margin:8px 0;font-size:12px}
  .zr-polish-result{font-size:13px;line-height:1.7}
  .zr-verdict{background:#ecf3ff;border-radius:7px;padding:9px 11px;margin-bottom:8px}
  .zr-p-flaw{border:1px solid #ffd9d9;background:#fff6f6;border-radius:7px;padding:8px 11px;margin-bottom:6px;font-size:12.5px}
  .zr-pf-name{font-weight:700;color:#ef4444}
  .zr-pf-claim{color:#b91c1c;margin:3px 0}
  .zr-pf-why{color:#444}
  .zr-p-rewrite{background:#e7f8f0;border:1px solid #bfe9d5;border-radius:7px;padding:10px 12px;color:#14532d;line-height:1.7}
  .zr-copy{margin-left:auto;background:none;border:1px solid #ddd;border-radius:6px;cursor:pointer;padding:2px 10px;font-size:12px}
`;
...
async function callApi(path, body) {
  ...
  if (data && data.ok === false) throw new Error((data.error && data.error.message) || "后端错误");
  return data;
}
...
// 起不来（后端没开、浏览器不支持流）就直接抛，调用方退回整包分析。
...
async function fetchComments(target) {
  const all = []; let offset = 0; const LIMIT = 20;
  ...
  for (let p = 0; p < MAX_PAGES; p++) {
    ...
    if (!list.length) { if (p === 0) throw new Error(page.diag || "没抓到评论"); break; }
...
// cls 必须和 CSS 里的 .zx-stance.agree / .oppose / .neutral 对上。
// 以前写成 "stance-agree"，没有规则命中，立场标签一直是灰的。
...
function stanceBarHtml(stats) {
  ...
  if (total === 0) return "";
  const ap = Math.round(agree / total * 100), op = Math.round(oppose / total * 100), np = 100 - ap - op;
...
function relatedPostsHtml(posts) {
  ...
  return `<div class="zr-count">🔗 相关帖子推荐：</div>` + posts.map(p => {
    const title = p.url ? `<a href="${esc(p.url)}" target="_blank" style="color:#0066ff;text-decoration:none">📄 ${esc(p.title)}</a>` : `📄 ${esc(p.title)}`;
    return `<div class="zr-comment"><div class="zr-c-text">${title}</div><div class="zr-count" style="color:#8590a6">${esc(p.reason || "")}</div></div>`;
  }).join("");
}
...
function openPanel() { const old = document.getElementById("zha-real-panel"); if (old) old.remove(); const p = document.createElement("div"); p.id = "zha-real-panel"; document.body.appendChild(p); return p; }

// 问题页：把页面上抓到的前 N 个回答逐个分析，每个回答配最多 5 条高赞评论。
// 和回答页一样「立即开面板 + 流式边出边显」——
// 以前是全部跑完才 openPanel()，用户盯着一个只会变字的按钮好几分钟，会以为插件挂了。
async function analyzeQuestion(qid, btn) {
  ...
  for (let i = 0; i < topAnswers.length; i++) {
    ...
    catch (e) { console.warn("抓评论失败，只分析正文", e); }
    ...
    try {
      ...
      try {
        const data = await callApi("/api/analyze", { comments: items });
        const byId = new Map();
...
function scrapePostContent() {
  ...
  for (const el of els) { const t = el.textContent.replace(/\s+/g, " ").trim(); if (t.length > 50) return t.slice(0, 3000); }
...
async function doMindmap(btn) {
  ...
  if (!content) { content = prompt("没自动识别到正文，请粘贴正文："); if (!content || !content.trim()) return; }
  const titleEl = document.querySelector("[class*='QuestionHeader-title'], h1, [class*='Post-Title']");
  const title = titleEl ? titleEl.textContent.replace(/\s+/g, " ").trim() : "";
  btn.disabled = true; btn.textContent = "生成中…";
  try {
    const data = await callApi("/api/mindmap", { title, content });
...
async function doPolish() {
  const draft = document.querySelector(".zr-polish-draft").value.trim();
  const resultEl = document.querySelector(".zr-polish-result");
  if (!draft) { resultEl.textContent = "输入评论，AI 实时帮你挑逻辑漏洞。"; return; }
  try {
    const d = await callApi("/api/polish", { draft });
    const flaws = (d.flaws || []).map(f => `<div class="zr-p-flaw"><div class="zr-pf-name">⚠ ${esc(f.name)}</div><div class="zr-pf-claim">问题点：${esc(f.claim)}</div><div class="zr-pf-why">原因：${esc(f.why)}</div></div>`).join("");
    resultEl.innerHTML = `<div class="zr-verdict"><b>判断：</b>${esc(d.verdict)}</div>${flaws}<div class="zr-p-rewrite"><b>✨ 改好的版本</b> <button class="zr-copy">复制</button><br>${esc(d.rewrite)}</div>`;
    const cp = resultEl.querySelector(".zr-copy");
    if (cp) cp.onclick = () => { navigator.clipboard.writeText(d.rewrite || "").then(() => {}).catch(() => {}); };
  } catch (e) { resultEl.textContent = "分析失败：" + e.message; }
}
...
function mountPolish() {
  ...
  btn.id = "zha-polish-btn";
  btn.textContent = "✍️ 写评论";
  btn.onclick = () => { panel.style.display = panel.style.display === "block" ? "none" : "block"; };
  document.body.appendChild(btn);

  panel.querySelector(".zr-close").onclick = () => { panel.style.display = "none"; };
  const ta = panel.querySelector(".zr-polish-draft");
  ta.addEventListener("input", () => {
    if (document.getElementById("zr-realtime").checked) {
      if (polishTimer) clearTimeout(polishTimer);
      polishTimer = setTimeout(doPolish, 1500);
    }
  });
  document.getElementById("zr-realtime").addEventListener("change", (e) => {
...
function mountStories() {
  const btn = document.createElement("button");
  btn.style.cssText = "position:fixed;right:24px;bottom:186px;z-index:2147483000;border:none;border-radius:22px;font-size:13px;font-weight:600;cursor:pointer;padding:10px 16px;background:#e65100;color:#fff;box-shadow:0 4px 14px rgba(0,0,0,.2)";
  btn.textContent = "📚 故事/知识";
  btn.onclick = async () => {
    ...
    finally { btn.disabled = false; btn.textContent = "📚 故事/知识"; }
  };
  document.body.appendChild(btn);
}
...
function renderStories(s, k) {
  const panel = openPanel();
  const sItems = (s && (s.data || s.items)) || [];
  const kItems = (k && (k.data || k.items)) || [];
  let html = `<div class="zr-head">📚 黑客松故事/知识 <button class="zr-close">✕</button></div>`;
  html += `<div class="zr-count">📖 知乎故事（${sItems.length}）</div>`;
  for (const it of sItems) html += `<div class="zr-comment" data-wid="${esc(it.work_id)}" style="cursor:pointer"><div class="zr-c-text">${esc(it.title)}</div><div class="zr-count" style="color:#8590a6">${esc(it.description || "")}</div></div>`;
  html += `<div class="zr-count">📚 知乎知识（${kItems.length}）</div>`;
  for (const it of kItems) html += `<div class="zr-comment" data-wid="${esc(it.work_id)}" style="cursor:pointer"><div class="zr-c-text">${esc(it.title)}</div><div class="zr-count" style="color:#8590a6">${esc(it.description || "")}</div></div>`;
  panel.innerHTML = html;
  panel.querySelector(".zr-close").onclick = () => panel.remove();
  panel.querySelectorAll("[data-wid]").forEach(el => {
    el.onclick = async () => {
      try {
        const d = await (await fetch(API + "/api/story/" + el.getAttribute("data-wid"))).json();
        showStoryDetail(d);
      } catch (e) { alert("出错：" + e.message); }
    };
  });
...
function showStoryDetail(d) {
  const item = (d && (d.data || d)) || {};
  const content = item.content || item.introduction || "";
  const panel = openPanel();
  let html = `<div class="zr-head">📖 ${esc(item.chapter_name || item.title || "")} <button class="zr-close">✕</button></div>`;
  ...
  html += `<button id="zr-mm" style="margin-top:10px;padding:8px 16px;background:#00897b;color:#fff;border:none;border-radius:18px;cursor:pointer;font-size:13px">🧠 生成思维导图</button>`;
  panel.innerHTML = html;
  panel.querySelector(".zr-close").onclick = () => panel.remove();
  panel.querySelector("#zr-mm").onclick = async (e) => {
    const b = e.target; b.textContent = "生成中…";
    try {
      const data = await callApi("/api/mindmap", { title: item.chapter_name || item.title || "", content });
      const p2 = openPanel();
      p2.innerHTML = `<div class="zr-head">🧠 思维导图 <button class="zr-close">✕</button></div><div class="zr-core">${data.mindmap ? mindmapHtml(data.mindmap, 0) : "无数据"}</div>`;
      p2.querySelector(".zr-close").onclick = () => p2.remove();
    } catch (err) { alert("出错：" + err.message); }
    finally { b.textContent = "🧠 生成思维导图"; }
  };
}
    ...
    jobs.push(
      (async () => {
        ...
        try {
          ...
          try {
            const data = await callApi("/api/analyze", { comments: page.comments });
            const byId = new Map();
            (data.analyses || []).forEach((a) => byId.set(String(a.id), a));
            for (const [id, c] of pending) {
              appendCommentCard(listEl, c, byId.get(id) ? cardHtml(byId.get(id)) : NOT_ANALYZED);
...
function mount() {
  ...
  const btn = document.createElement("button");
  btn.id = "zha-real-btn";
  btn.textContent = "⚔️ 分析评论";
  btn.onclick = async () => {
    btn.disabled = true; btn.textContent = "抓取中…";
    try {
      const target = detectTarget();
      if (!target) { alert("无法识别当前页面。请打开回答页、文章页或问题页。"); return; }
      if (target.type === "question") { await analyzeQuestion(target.id, btn); return; }
      await analyzeIncremental(target, btn);
    } catch (e) { console.error(e); alert("出错：" + e.message); }
    finally { btn.disabled = false; btn.textContent = "⚔️ 分析评论"; }
  };
  document.body.appendChild(btn);
