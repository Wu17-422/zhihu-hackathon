...
接口三个：
    POST /api/analyze   评论区每条评论 → 立场 / 意思 / 逻辑 / 漏洞 + 相关帖子推荐
    POST /api/polish    用户自己写的草稿 → 挑逻辑漏洞 + 给改法
...
from mock import mock_mindmap, mock_polish, mock_result
from zhida import (
    ZhidaError,
    analyze_comments,
    generate_mindmap,
...
def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "POST /api/polish": "用户自己写的评论 → 挑逻辑漏洞 + 给改法",
            "POST /api/mindmap": "把知乎帖子/回答转成思维导图",
